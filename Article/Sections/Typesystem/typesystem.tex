...
\begin{align*}
\tag{Valid Operation} &\infrule{\valid{T'}}{\valid{T\ra T'}}\\
\tag{Valid Boolean} &\valid{bool}\\
\tag{Valid Basic} &\valid{B^l}\\
\tag{Valid Channel} &\infrule{\valid{U^l}\quad l\leq l'}{\valid{ch[U^l]^{l'}}}\\
\tag{Valid Tuple} &\infrule{\forall i\ \valid{T_i}\quad \forall i\ \level{T_i}\leq l}{\valid{{T_1 \times...\times T_k}^l}}& where\ 1\leq i\leq k
\end{align*}
...
%\ensuremath{\frac{P \ra wrong}{P \para Q \ra wrong}}
