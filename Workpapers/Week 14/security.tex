...
E hold for nil, simply because nil do not do anything.

\fig{\frac{E(x) \vdash ch^l[T]\quad E,y:T\vdash P}{E \vdash \inp{x}{y}P}}{Type rule for input.}{typeinp}
E holds for input when \textit{x} is of type channel and E holds for \textit{y} in \textit{P}.

\fig{\frac{E(x) \vdash ch^l[T] \quad E \vdash M:T}{E\vdash \lift{x}{M}}}{Type rule for lift.}{typelift}
E holds for lift when \textit{x} is of type channel and E hold for \textit{M}
...
\fig{\frac{E\vdash \phi \quad E\vdash P}{E \vdash [\phi] P}}{Type rule for boolean arithmetics.}{typebool}
